...
/**
 * Course ids that actually have questions filed under them.
 *
 * This is the raw bank, so honors and other borrowing courses do NOT appear
 * here under their own id: callers must resolve through courses.bankFor first.
 */
function stockedBanks() {
  return new Set(
    getDb().prepare('SELECT DISTINCT course_id FROM questions WHERE course_id IS NOT NULL')
      .all().map((r) => r.course_id)
  );
}

module.exports = { seed, stockedBanks, pruneRemovedCourses, loadQuestionFiles, getQuestion, forClient, hydrate, countQuestions };
