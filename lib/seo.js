...
function courseFaq(course, coverage) {
  ...
  return [
    ...
    },
    {
      ...
      a: `The free plan covers a few questions a day in one subject, which is enough to work through ${course.name} slowly. Premium removes the daily cap for $4.99 a month or $29.99 a year and unlocks every other course at the same time.`,
    },
  ];
}
...
function coursePageHtml({ course, coverage, origin, totals }) {
  const canonical = `${origin}/courses/${course.id}`;
  const unitsWithContent = coverage.units.filter((u) => u.questions > 0);
...
<main class="shell">
  ...
  <article class="seo-article">
    ...
    <p><a class="btn btn-primary" href="/">Start practising free</a>
      <span class="dim">5 questions a day free, no card required.</span></p>
