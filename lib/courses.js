 ...
 * courses it chose to build. Keen maps the actual curriculum a student is
...
function loadAll() {
  ...
  for (const file of files) {
    ...
    for (const course of parsed) {
      ...
      courses.push({
        ...
        // Honors sections cover the same syllabus as the regular course at
        // greater depth, so they reuse that course's question bank rather than
        // duplicating ~300 questions each. adaptive.js resolves this alias and
        // biases selection away from easy questions.
        sharesBankWith: course.sharesBankWith || null,
  ...
  // A shared bank that points nowhere would leave the course silently empty,
  // which is far harder to notice than a startup error.
  for (const c of courses) {
    if (c.sharesBankWith && !ids.has(c.sharesBankWith)) {
      errors.push(`${c.id} sharesBankWith unknown course: ${c.sharesBankWith}`);
    }
  }
...
/**
 * The course whose question bank a given course actually draws from.
 *
 * Honors sections reuse the regular course's questions (see sharesBankWith), so
 * every query that filters questions or cards by course_id must resolve through
 * here. Skipping it makes an honors course look empty: the rows exist, they are
 * just filed under the regular course id.
 */
function bankFor(courseId) {
  const course = getCourse(courseId);
  return (course && course.sharesBankWith) || courseId;
}
...
function suggestedForGrade(grade) {
  ...
  return allCourses()
    ...
    .sort((a, b) => {
      const levelRank = { regular: 0, honors: 1, ap: 2, 'test-prep': 3 };
...
module.exports = {
  ...
  bankFor,allCourses, getCourse, getUnit, allUnits, byCategory,
