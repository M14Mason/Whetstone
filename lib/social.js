...
/** Re-point a set at a different class, or at none. */
function setSetCourse(setId, userId, courseId) {
  const db = getDb();
  ...
  if (courseId && !courses.getCourse(courseId)) throw new SocialError('Unknown course.');
  db.prepare('UPDATE custom_sets SET course_id = ?, updated_at = ? WHERE id = ?')
    .run(courseId || null, new Date().toISOString(), setId);
  return getSet(setId, userId);
}
...
function fileBug(userId, { title, body, page, userAgent, appVersion, kind = 'bug', questionId = null }) {
  ...
  const result = getDb().prepare(`
    INSERT INTO bug_reports (user_id, title, body, page, user_agent, app_version, kind, question_id, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    userId || null, t.slice(0, 140), b.slice(0, 4000),
    String(page || '').slice(0, 200), String(userAgent || '').slice(0, 300),
    String(appVersion || '').slice(0, 40),
    kind === 'question' ? 'question' : 'bug',
    questionId ? String(questionId).slice(0, 120) : null,
    new Date().toISOString()
  );
  return { id: Number(result.lastInsertRowid) };
}

/** The reasons the report sheet offers, and the wording that gets stored. */
const QUESTION_REPORT_REASONS = {
  'wrong-answer': 'The marked answer is wrong',
  'bad-explanation': 'The explanation is wrong or does not match',
  'typo': 'Typo or formatting problem',
  'confusing': 'The question is unclear or ambiguous',
  'off-topic': 'Not part of this course or unit',
};

/**
 * Report a specific question.
 *
 * Separate from fileBug's free-text form on purpose: a student mid-question
 * will tap a reason, and will not write a paragraph. The question id, its
 * prompt and the marked answer are captured here so the report is actionable
 * without going back to ask the student what they were looking at.
 */
function reportQuestion(userId, { questionId, reason, note, page, userAgent, appVersion }) {
  const id = String(questionId || '').trim();
  if (!id) throw new SocialError('No question to report.');

  const label = QUESTION_REPORT_REASONS[reason];
  if (!label) throw new SocialError('Pick a reason for the report.');

  const row = getDb().prepare('SELECT prompt, choices, answer, explanation, course_id, unit FROM questions WHERE id = ?').get(id);
  if (!row) throw new SocialError('That question no longer exists.');

  let marked = '';
  try {
    const choices = JSON.parse(row.choices || '[]');
    marked = choices[row.answer] != null ? String(choices[row.answer]) : '';
  } catch { marked = ''; }

  const body = [
    `Reason: ${label}`,
    note ? `Student note: ${String(note).trim().slice(0, 1000)}` : null,
    '',
    `Question id: ${id}`,
    `Course: ${row.course_id || 'unassigned'}${row.unit ? ` / ${row.unit}` : ''}`,
    `Prompt: ${row.prompt}`,
    `Marked correct: ${marked}`,
    `Explanation: ${row.explanation || '(none)'}`,
  ].filter((line) => line !== null).join('\n');

  return fileBug(userId, {
    title: `Question report: ${label}`,
    body,
    page,
    userAgent,
    appVersion,
...
function listBugs({ status = null, limit = 200 } = {}) {
  ...
  return rows.map((r) => ({
    ...
    kind: r.kind || 'bug', questionId: r.question_id || null,
  }));
}
...
module.exports = {
  ...
  createSet, replaceCards, setSetCourse, getSet, listSets, deleteSet,
  fileBug, listBugs, myBugs, setBugStatus, reportQuestion, QUESTION_REPORT_REASONS,
