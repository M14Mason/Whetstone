...
CREATE TABLE IF NOT EXISTS bug_reports (
  ...
  -- 'bug' for anything broken, 'question' for "this answer is wrong".
...
CREATE TABLE IF NOT EXISTS study_groups (
  ...
  -- Invite codes are short lived. See lib/groups.js INVITE_TTL_MS.
...
-- Proof that a person agreed to be charged again, before we ever charged them.
--
-- California's Automatic Renewal Law (as amended by AB 2863, in force since
-- 1 July 2025) and the federal ROSCA both require express affirmative consent
-- to the renewal terms, given separately from any other agreement, before
-- billing information is taken. AB 2863 additionally requires the seller to
-- KEEP proof of that consent for three years, or one year after the
-- subscription ends, whichever is longer.
--
-- "We showed them a Terms link" is not proof. This table is the proof: it
-- stores the exact disclosure text the person was shown, the hash the client
-- echoed back to prove it rendered that text and not something friendlier,
-- and the moment they ticked the box.
--
-- Deliberately NOT ON DELETE CASCADE. If someone deletes their account we
-- still need the consent record to answer a chargeback or a regulator, and it
-- is the one thing a deleted user has a weaker claim to erase, because it is
-- our record of a transaction rather than a profile about them. user_id is
-- nulled on delete instead, keeping the row without keeping the person.
CREATE TABLE IF NOT EXISTS renewal_consents (
  id              INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id         INTEGER,
  email           TEXT NOT NULL,
  plan            TEXT NOT NULL,
  ...
  payer_attested  INTEGER NOT NULL DEFAULT 0,
  consented_at    TEXT NOT NULL,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS idx_renewal_consents_user ON renewal_consents(user_id);
`;
...
const MIGRATIONS = [
  ...
  {
    id: '007-question-reports',
    up(handle) {
      const cols = handle.prepare('PRAGMA table_info(bug_reports)').all().map((c) => c.name);
      // "This answer is wrong" is a different kind of report from "the page
      // broke", and it is only actionable with the question id attached. Both
      // still live in one table so there is one place to read reports from.
      if (!cols.includes('kind')) {
        handle.exec("ALTER TABLE bug_reports ADD COLUMN kind TEXT NOT NULL DEFAULT 'bug'");
      }
      if (!cols.includes('question_id')) {
        handle.exec('ALTER TABLE bug_reports ADD COLUMN question_id TEXT');
      }
    },
  },
  {
    id: '006-pending-cancellation',
    up(handle) {
      const cols = handle.prepare('PRAGMA table_info(users)').all().map((c) => c.name);
      // When a subscription is set to cancel at period end, Stripe keeps it
      // active until that date. Without recording the date, the app cannot
      // tell a customer their cancellation worked, which is how a cancelled
      // customer ends up filing a chargeback instead.
      if (!cols.includes('premium_cancels_at')) {
        handle.exec('ALTER TABLE users ADD COLUMN premium_cancels_at TEXT');
      }
    },
  },
  {
    id: '005-referrals-and-timed-premium',
    up(handle) {
      const cols = handle.prepare('PRAGMA table_info(users)').all().map((c) => c.name);
      // A referral code per account, allocated lazily on first use.
      if (!cols.includes('referral_code')) handle.exec('ALTER TABLE users ADD COLUMN referral_code TEXT');
      // Premium granted as time rather than bought. Referral rewards and any
      // future promo credit land here, so users.plan keeps meaning exactly one
      // thing: there is a paid subscription behind it.
      if (!cols.includes('premium_until')) handle.exec('ALTER TABLE users ADD COLUMN premium_until TEXT');
      handle.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_referral_code ON users(referral_code) WHERE referral_code IS NOT NULL');

      handle.exec(`CREATE TABLE IF NOT EXISTS referrals (
        id          INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      handle.exec('CREATE INDEX IF NOT EXISTS idx_referrals_referrer ON referrals(referrer_id)');
    },
  },
  {
...
function addMissingColumns(db) {
  ...
  // Short-lived group invite codes. Existing groups get a NULL expiry, which
  // lib/groups.js treats as already expired, so every group has to mint a fresh
  // code. That is the safe direction: the alternative is that codes handed out
  // before this change stay valid forever.
  if (!has('study_groups', 'invite_expires_at')) {
    db.exec('ALTER TABLE study_groups ADD COLUMN invite_expires_at TEXT');
  }

  // Sessions used to store the raw token. Hashing new ones does nothing about
  // the plaintext tokens already sitting in the database -- each of those is
  // still a working credential until it expires. Clear them once.
  //
  // The visible effect is that everyone signed in at the time of this deploy
  // has to sign in again. That is the correct trade: a one-off login beats
  // leaving live credentials in plaintext on disk.
  const done = db.prepare("SELECT value FROM schema_meta WHERE key = 'sessions_hashed'").get();
  if (!done) {
    db.exec('DELETE FROM sessions');
    db.prepare('INSERT INTO schema_meta (key, value) VALUES (?, ?)')
      .run('sessions_hashed', new Date().toISOString());
  }
