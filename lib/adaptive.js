...
function recordResult(userId, question, isCorrect, options = {}) {
  // `mode` decides which free allowance this attempt spends.
  const { chosen = null, now = new Date(), mode = 'learn' } = options;
  const db = getDb();
...
/**
 * The learning profile: what Keen has worked out about this specific
