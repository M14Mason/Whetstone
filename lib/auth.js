 ...
 *
 * Required, not optional. CCPA gives Californians a right to deletion, GDPR
 * calls it erasure, and the amended COPPA rule expects operators to actually be
 * able to remove a child's data rather than merely promise to. Our Privacy
 * Policy already told students they could ask us to delete everything; until
 * now there was no code that could do it.
 *
 * Every table referencing users(id) is ON DELETE CASCADE and PRAGMA
 * foreign_keys is ON, so one DELETE removes sessions, attempts, topic state,
 * custom sets and their cards, group membership, and progression. Bug reports
 * are ON DELETE SET NULL on purpose: the report stays so a real fault does not
 * vanish, but it is no longer attached to a person.
 *
 * Returns a summary so the caller can tell the user what actually went.
 */
function deleteAccount(userId) {
  const db = getDb();
  const before = {
    attempts: db.prepare('SELECT COUNT(*) n FROM attempts WHERE user_id = ?').get(userId).n,
    sets: db.prepare('SELECT COUNT(*) n FROM custom_sets WHERE user_id = ?').get(userId).n,
    sessions: db.prepare('SELECT COUNT(*) n FROM sessions WHERE user_id = ?').get(userId).n,
  };
