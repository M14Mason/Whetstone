 ...
 */
/**
 * The user this event belongs to, by metadata when present and by Stripe
 * customer id otherwise. Subscription events carry no metadata of ours, so the
 * customer id is usually the only link back.
 */
function resolveUserId(userId, customerId) {
  if (Number.isInteger(userId) && userId > 0) return userId;
  if (!customerId) return null;
  const row = getDb().prepare('SELECT id FROM users WHERE stripe_customer_id = ?').get(customerId);
...
function applyWebhookEvent(event) {
  ...
  // cancel_at_period_end and keeps it active until the period ends, sending
  // customer.subscription.updated. Ignoring that event was why cancelling
  // appeared to do nothing: the account still said Premium, with no end date
  // and no acknowledgement, right up until it silently expired.
  if (type === 'customer.subscription.updated') {
    const target = resolveUserId(userId, object.customer);
    if (target) {
      const endsAt = object.cancel_at || object.current_period_end;
      if (object.cancel_at_period_end && endsAt) {
        getDb().prepare('UPDATE users SET premium_cancels_at = ? WHERE id = ?')
          .run(new Date(endsAt * 1000).toISOString(), target);
        return { handled: true, action: 'cancellation_scheduled' };
      }
      // Resumed inside the notice period: clear the pending cancellation.
      getDb().prepare('UPDATE users SET premium_cancels_at = NULL WHERE id = ?').run(target);
      return { handled: true, action: 'cancellation_cleared' };
    }
  }
  ...
  if (type === 'customer.subscription.deleted') {
    ...
    if (Number.isInteger(userId) && userId > 0) {
      ...
      getDb().prepare('UPDATE users SET premium_cancels_at = NULL WHERE id = ?').run(userId);
 ...
 */
/**
 * Send a paying customer to Stripe's hosted billing portal to cancel.
 *
 * Cancellation was simply not implemented for live billing. cancelDemo() threw
 * and told people to use "the Stripe customer portal", which did not exist
 * anywhere in the app, and the Cancel button was hidden whenever billing was
 * live. So the moment real money started moving, nobody could stop paying,
 * while the Terms promised "Cancel any time from Settings". That is a promise
 * the software could not keep.
 *
 * Stripe's portal rather than a bespoke cancel button, because the portal also
 * handles updating a card, viewing invoices, and switching monthly to annual,
 * and Stripe keeps it correct as their API changes. Cancellation there defaults
 * to end-of-period, which is exactly what our Terms describe.
 */
async function openBillingPortal(user) {
  if (!isBillingLive()) {
    throw new BillingError('Billing is in demo mode; there is nothing to manage.', 400);
  }
  const row = getDb().prepare('SELECT stripe_customer_id FROM users WHERE id = ?').get(user.id);
  const customer = row && row.stripe_customer_id;
  if (!customer) {
    // Happens if the plan was set by hand, or if checkout.session.completed
    // never arrived. Say so plainly rather than showing a broken button.
    throw new BillingError(
      'We cannot find a Stripe customer for this account. Email support and we will cancel it for you.',
      400
    );
  }
  ...
  const session = await stripeRequest('/billing_portal/sessions', {
    customer,
    return_url: `${config.publicUrl}/?billing=managed`,
  });
