...
function scopeClause(scope = {}) {
  ...
  // Honors ids resolve to the bank they share (see courses.bankFor).
  if (scope.courseId) { where.push('course_id = ?'); params.push(courses.bankFor(scope.courseId)); }
  if (scope.unit) { where.push('unit = ?'); params.push(scope.unit); }

  if (Array.isArray(scope.courseIds) && scope.courseIds.length > 0) {
    // Same bank resolution as above: without it, flashcards, match and tests
    // come back empty for an honors course even though Learn works.
    const banks = [...new Set(scope.courseIds.map(courses.bankFor))];
    where.push(`course_id IN (${banks.map(() => '?').join(',')})`);
    params.push(...banks);
  }
