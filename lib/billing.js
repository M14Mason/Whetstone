...
/**
 * Start a Premium ($8.99/mo) subscription.
 */
async function startPremiumCheckout(user) {
  if (!isBillingLive()) {
    setUserPlan(user.id, 'premium');
    return { mode: 'demo', url: null, message: 'Demo mode: Premium unlocked locally. No payment was taken.' };
  }
  if (!config.stripe.pricePremiumMonthly) {
    throw new BillingError('STRIPE_PRICE_PREMIUM_MONTHLY is not configured.', 500);
  }
  ...
  const session = await stripeRequest('/checkout/sessions', {
    ...
    line_items: [{ price: config.stripe.pricePremiumMonthly, quantity: 1 }],
    metadata: { user_id: String(user.id), kind: 'premium' },
  });
