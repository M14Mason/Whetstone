...
function effectivePlan(userId) {
  ...
  // Premium granted as time rather than bought: referral rewards, promo credit.
  // Checked before group membership so someone holding both keeps the clearer
  // label, and it lapses on its own with no cancellation to process.
  if (user.premium_until && new Date(user.premium_until) > new Date()) {
    return config.plans.premium;
  }
...
/**
 * Free tier: a per-mode daily quota (see config.freeDailyLimits), one subject at a time.
...
/**
 * Everything the class board needs, per enrolled course, in one pass.
 *
 * Mastery is the share of the course's touched topics that have reached the
 * mastered threshold. A course with no attempts reports null rather than 0 so
 * the UI can say "Not started" instead of implying the student failed it.
 */
function courseBoard(userId) {
  const db = getDb();
  const enrolled = userCourses(userId);

  return enrolled.map((c) => {
    const bank = courses.bankFor(c.id);
    const course = courses.getCourse(c.id);

    const stats = db.prepare(`
      SELECT COUNT(*) AS attempts, MAX(a.answered_at) AS last
      FROM attempts a JOIN questions q ON q.id = a.question_id
      WHERE a.user_id = ? AND q.course_id = ?
    `).get(userId, bank) || {};

    // Topic states carry the mastery signal; join them to this course's topics.
    const topics = db.prepare(`
      SELECT DISTINCT topic FROM questions WHERE course_id = ?
    `).all(bank).map((r) => r.topic);
    ...
    if (topics.length) {
      ...
      // Same signal and thresholds the adaptive engine uses in masteryStatus,
      // so the board never disagrees with the dashboard.
      const states = db.prepare(
        `SELECT topic, attempts, ewma_correct FROM topic_state WHERE user_id = ? AND topic IN (${ph})`
      ).all(userId, ...topics);
      touched = states.length;
      for (const st of states) {
        if (st.attempts < 3) continue;              // still 'learning'
        if (st.ewma_correct >= 0.85) mastered += 1;
        else if (st.ewma_correct < 0.6) weak += 1;
      }
    }

    // The next unit worth doing: first one with content the student has not
    // finished. Falls back to the first unit for a fresh course.
    let nextUnit = null;
    if (course) {
      for (const u of course.units) {
        const n = db.prepare('SELECT COUNT(*) AS n FROM questions WHERE course_id = ? AND unit = ?')
          .get(bank, u.name).n;
        if (n > 0) { nextUnit = u.name; break; }
      }
    }
    ...
    return {
      id: c.id,
      name: c.name,
      level: course ? course.level : 'regular',
      levelLabel: course ? course.levelLabel : 'Regular',
      category: course ? course.category : 'Other',
      attempts: stats.attempts || 0,
      lastStudied: stats.last || null,
      masteryPercent: touched === 0 ? null : Math.round((mastered / touched) * 100),
      weakCount: weak,
      nextUnit,
      unitCount: course ? course.units.length : 0,
    };
  });
}
