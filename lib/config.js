...
const config = {
  ...
  // Shown to a user when we cannot email them. Until RESEND_API_KEY is set,
  // this address is the ONLY way somebody who forgets their password can get
  // back in, so it has to be real and monitored.
  supportEmail: process.env.SUPPORT_EMAIL || 'masonngo70@gmail.com',

  // The one account that may read /admin. Defaults to the support address so
  // the dashboard works without another secret to set, and is matched against
  // a SIGNED-IN, EMAIL-CONFIRMED account: knowing the address is not enough.
  adminEmail: (process.env.ADMIN_EMAIL || process.env.SUPPORT_EMAIL || 'masonngo70@gmail.com')
    .trim().toLowerCase(),
  ...
  freeDailyLimits: {
    // 15, not 3.
    //
    // The pitch is that Keen finds the units you keep getting wrong. The
    // weakness model needs roughly ten answers before it can say anything
    // true, so a 3/day cap meant a student had to come back four days running
    // before the product did the one thing it promises. Most never did: they
    // hit the wall having seen a quiz, not a diagnosis.
    //
    // 15 is about one real study session. It is still a wall during actual
    // exam cramming, which is exactly the moment upgrading is worth it, so the
    // paid tier still has a job. The free tier's real limits are now one
    // subject and the locked modes, not a starvation cap.
    learn: intEnv('FREE_LEARN_LIMIT', 15),
    // Review scales with Learn: reviewing mistakes is the habit worth forming,
    // and capping it below the number of mistakes a session produces is
    // self-defeating.
    review: intEnv('FREE_REVIEW_LIMIT', 10),
  },

  // Bump when Terms or the Privacy Policy change materially. Stamped onto
  // every acceptance and every renewal consent, so a record from last year
  // says which version of the document the person actually agreed to.
  // Lives here rather than in server.js because lib/consent.js needs it too
  // and libraries must not reach up into the server.
  tosVersion: '2026-08-11',
  ...
  plans: {
    free: {
      ...
      dailyQuestionLimit: intEnv('FREE_LEARN_LIMIT', 15),
...
/**
 * Is billing actually usable, not just partly configured?
 *
 * This used to return true as soon as STRIPE_SECRET_KEY existed. That is the
 * dangerous half-state: /api/health would report "live", the Upgrade button
 * would stop granting Premium for free, and then checkout would fail because
 * there was no price to sell. The app would look ready and take nobody's money.
 *
 * A price ID is required too. The monthly price is the minimum viable product;
 * annual and group seats are optional and degrade gracefully on their own.
 */
function isBillingLive() {
  return Boolean(config.stripe.secretKey) && Boolean(config.stripe.pricePremiumMonthly);
}
